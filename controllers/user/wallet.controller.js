...
import { getWalletService, creditWalletService } from '../../services/wallet.service.js';
...
export const getWallet = asyncHandler(async (req, res) => {
  ...
  if (!result.success) {
    return res.status(500).render('error', { message: result.message, layout: 'layouts/user' });
  }
...
export const verifyTopup = asyncHandler(async (req, res) => {
  ...
  if (!pending || pending.razorpayOrderId !== razorpay_order_id) {
    return res.status(400).json({ success: false, message: 'Invalid top-up session. Please try again.' });
  }
